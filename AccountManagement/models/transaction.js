...
Transaction.Search = function Search(data, order_constraints, callback){
    ...
    for (var i in data){
        ...
        if((order_constraints.low_money != "" && (parseFloat(data[i].money) < parseFloat(order_constraints.low_money)))||(order_constraints.upper_money != "" && (parseFloat(data[i].money) > parseFloat(order_constraints.upper_money)))){
            continue;
        }
