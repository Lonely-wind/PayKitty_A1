...
User.prototype.save = function  save(callback) {
    ...
    //uuid = uid.v4();
    //插入数据库
    var sql ="insert into UserAccount (AccountName,Password,Name,Phone,Email,State,Type,IdNo,Balance,PayPassword) values(?,?,?,?,?,?,?,?,?,?)";

    mysql.query(sql,[user.name,user.password,user.realname,user.phone,user.email,0,user.Type,user.IdNo,0,user.password],function(err,results,fields){
        if (err) {
            throw err;
        } else {
            //返回用户id
